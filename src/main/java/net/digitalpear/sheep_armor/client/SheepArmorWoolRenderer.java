...
public class SheepArmorWoolRenderer extends FeatureRenderer<SheepEntity, SheepEntityModel<SheepEntity>> {
    ...
    public SheepArmorWoolRenderer(FeatureRendererContext<SheepEntity, SheepEntityModel<SheepEntity>> context, EntityModelLoader loader) {
        super(context);
        this.model = new SheepEntityModel<>(loader.getModelPart(SAModelLayers.SHEEP_ARMOR_WOOL));
    }
    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, SheepEntity sheepEntity, float f, float g, float h, float j, float k, float l) {
        if (((SheepArmorAccess) sheepEntity).hasArmor()) {
            ...
            if (var13 instanceof SheepArmorItem animalArmorItem) {
                ...
                VertexConsumer vertexConsumer = vertexConsumerProvider.getBuffer(RenderLayer.getEntityCutoutNoCull(animalArmorItem.getWoolTexture()));
